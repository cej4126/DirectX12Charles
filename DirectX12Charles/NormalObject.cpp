...
NormalObject::NormalObject(Graphics &gfx, std::string tag)
   :
   gfx(gfx),
   tag(tag),
...
std::shared_ptr<NormalObject> NormalObject::Resolve(Graphics &gfx, const std::string &tag)
{
   return Bind::BindableCodex::Resolve<NormalObject>(gfx, tag);
}

std::string NormalObject::GenerateUID(const std::string &tag)
{
   return typeid(NormalObject).name() + std::string("#") + tag;
}

std::string NormalObject::GetUID() const noexcept
{
   return GenerateUID(tag);
}

void NormalObject::Bind(Graphics &gfx) noexcept
{
   ...
   ID3D12DescriptorHeap *textureDescriptorHeaps[] = { textureDescriptorHeap.Get() };
...
void NormalObject::CreateRootSignature(bool constantFlag, bool materialFlag, bool textureFlag)
{
   ...
   // constant buffer for matrix pixel
   rootParameters[VIEW_PS_CB].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
...
void NormalObject::CreateShader(const std::wstring &vertexPath, const std::wstring &pixelPath)
{
...
void NormalObject::SetLightView(ID3D12Resource *mylightView)
{
   lightView = mylightView;
}

void NormalObject::LoadVerticesBuffer(const hw3dexp::VertexBuffer &vertices)
{
...
void NormalObject::CreateConstant(const XMFLOAT3 &colorBuffer, int size)
{
...
void NormalObject::LoadIndicesBuffer(const std::vector<unsigned short> &indices)
{
...
void NormalObject::CreateTexture(const Surface &surface, int slot)
{
   D3D12_HEAP_PROPERTIES heapProps;
   ZeroMemory(&heapProps, sizeof(heapProps));
   heapProps.Type = D3D12_HEAP_TYPE_DEFAULT;
